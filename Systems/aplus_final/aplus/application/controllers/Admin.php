...
class Admin extends CI_Controller
{
	...
	$this->user_authentication_admin();
		...
		$this->load->view('admin/admin_header');
		$this->load->view('admin/cpa', $data);
		$this->load->view('extra/js_admin_cpa');
		$this->load->view('extra/html_close');	
	}
	...
	function view_reg()
	{
		...
		$this->load->view('admin/admin_header');
		$this->load->view('admin/view_reg', $data);
		$this->load->view('extra/js');
		$this->load->view('extra/html_close');
	}
	...
	function cia_form_submit()
	{
		...
		if ($this->admin_misc_model->check_uniqueness($username))
		{
			...
			redirect(base_url() . 'admin/admin_header');
		}
	}

	function cra_form_submit()
	{
		$this->user_authentication_admin();
		...
		if ($this->admin_misc_model->check_uniqueness($username))
		{
			...
			$instId = $this->admin_misc_model->get_instructor_id($username);
			$this->admin_misc_model->insert_reg_role($instId[0]['userId']);

			// $dept = $this->input->post('dept');
			// $this->admin_misc_model->insert_lecturer_department($dept, $instId[0]['userId']);

			redirect(base_url() . 'admin/cra');
		}
	}

	function cpa_form_submit()
	{
		$this->user_authentication_admin();
		...
		if ($this->admin_misc_model->check_uniqueness($username))
		{
			...
			$instId = $this->admin_misc_model->get_instructor_id($username);
			$this->admin_misc_model->insert_princ_role($instId[0]['userId']);

			// $dept = $this->input->post('dept');
			// $this->admin_misc_model->insert_lecturer_department($dept, $instId[0]['userId']);

			redirect(base_url() . 'admin/cpa');
		}
	}
