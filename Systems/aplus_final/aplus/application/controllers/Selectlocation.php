...
class Selectlocation extends CI_Controller 
{
	...
	function login()
	{
		...
		$schoolnum = $this->input->post('school');

		$validate = $this->user_model->validate_user($username, $password);
		$userId = $validate->userId;


		//$validateschool=$this->user_model->validate_school($username,$schoolnum);
		...
		if ($validate != false)
		{
			...
			else if ($role->rolename == 'registrar')
				redirect(base_url().'registrar/index');
			else if ($role->rolename == 'principal')
	...
	function register()
	{
		...
		if ($this->user_model->check_regkey($key))
		{
			...
			$this->user_model->register($fname, $mname, $lname, $gender, $address, $city, $province, $zip, $phone, $email, $username, $password,$school);
			...
			$this->user_model->sectioninsert($id['userId'],$grdlvl);


			$this->user_model->update_regkey($key);

			//auto assign student to section:
