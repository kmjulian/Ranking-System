...
class Lecturer extends CI_Controller 
{
	...
	function computation_table()
	{
		...
		$data['termnum'] = $this->input->post('termnum');
	...
	function save_grades()
	{
		...
		//$level = $this->lecturer_misc_model->get_level($this->session->userdata('username'));
		...
		foreach ($grade_data as $student)
		{
			...
			$id = $this->lecturer_misc_model->save_final_grade($student[2],$student[0]);
	...
	function full_rank()
	{
		...
		$app_data = $this->data();
		$app_data['subjectname'] = $this->lecturer_misc_model->get_subject($this->session->userdata('username'));	
		$app_data['students'] = $this->lecturer_misc_model->getsubjectranks();

		$this->load->view('lecturer/navbar_sidebar', $app_data);
		$this->load->view('lecturer/fullrank');
		$this->load->view('extra/js');
		$this->load->view('extra/html_close');
	}
	...
	function powerbi()
	{
		$app_data = $this->data();
		$this->load->view('lecturer/navbar_sidebar', $app_data);
		$this->load->view('lecturer/powerbi');
		$this->load->view('extra/js');
		$this->load->view('extra/html_close');	
	}
